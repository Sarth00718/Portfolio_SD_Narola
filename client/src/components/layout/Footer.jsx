...
const SOCIAL = [
  { icon: Github, href: PROFILE.github, label: 'GitHub', color: 'hover:text-white' },
  { icon: Linkedin, href: PROFILE.linkedin, label: 'LinkedIn', color: 'hover:text-blue-400' },
  { icon: Mail, href: `mailto:${PROFILE.email}`, label: 'Email', color: 'hover:text-indigo-400' },
];
...
export default function Footer() {
  ...
  return (
    <footer
      className="border-t"
      style={{ background: 'rgba(10,15,30,0.97)', borderColor: 'rgba(255,255,255,0.07)' }}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <div className="font-display font-bold text-white">{PROFILE.name}</div>
                <div className="text-xs text-slate-500">{PROFILE.title}</div>
              </div>
            </div>
            <p className="text-xs text-slate-500 leading-relaxed mb-5 max-w-xs">
              Final-year B.Tech CSE student at Nirma University. Building production-ready web & AI systems. Open to full-time SDE roles in 2026.
            </p>
            {/* Social Icons */}
          ...
          {/* Navigation */}
          <div>
            <h4 className="text-xs font-bold text-slate-300 uppercase tracking-widest mb-5">Navigate</h4>
            ...
            <ul className="space-y-3">
              <li>
                <a href={`mailto:${PROFILE.email}`}
                  className="flex items-start gap-2 text-xs text-slate-500 hover:text-indigo-400 transition-colors">
        ...
        <div className="pt-8 border-t flex flex-col sm:flex-row items-center justify-between gap-3"
          ...
          <p className="flex items-center gap-1.5 text-xs text-slate-600">
            Built with <Code2 size={11} className="text-indigo-500" /> React + Vite + Tailwind CSS
          </p>
        </div>
      </div>
    </footer>
